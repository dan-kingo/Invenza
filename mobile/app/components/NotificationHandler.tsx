  ...
  useEffect(() => {
    ...
    const responseSubscription = Notifications.addNotificationResponseReceivedListener(response => {
      ...
      if (data.type === 'low_stock' || data.type === 'out_of_stock') {
        ...
        router.push(`/stock/item-detail?id=${data.itemId}`);
  ...
  useEffect(() => {
    if (notification) {
      ...
      if (data?.navigateTo) {
        ...
        buttons.unshift({ text: 'View', onPress: () => router.push('/(tabs)/stock') });
      } else if (data?.type === 'expiry_warning' && data?.itemId) {
        buttons.unshift({ text: 'View', onPress: () => router.push(`/stock/item-detail?id=${data.itemId}`) });
      }
