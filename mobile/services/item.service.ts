...
class ItemService {
  ...
  async scanItem(tagId: string): Promise<{ message: string; tag: any; item: Item , tagId: string, qrCode: string}> {
    ...
    if (!isOnline) {
      const items = await offlineService.getItems();
      const item = items.find(i => i.tags.includes(tagId) || i._localId === tagId);

      if (!item) {
        throw new Error('Item not found for this tag');
      }

      return {
        message: 'Item scanned offline',
        tag: { tagId },
        item,
        tagId,
        qrCode: '',
    ...
    try {
      ...
      console.log('Scanning item offline due to error');
      const items = await offlineService.getItems();
      const item = items.find(i => i.tags.includes(tagId) || i._localId === tagId);
